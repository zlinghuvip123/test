...
from ApiManager.utils.common import get_file_path, delete_folder_recursively
import mimetypes
...
def run_by_suite(index, base_url, path, cfg_id):
    ...
    for val in include:
        run_by_single(val[0], base_url, path, cfg_id)
...
def run_test_by_type(id, base_url, path, type, cfg_id):
    ...
    if type == 'project':
        ...
        run_by_single(id, base_url, path, cfg_id)


def run_sdk_test(test_file_path, test_name, sdk_name):
