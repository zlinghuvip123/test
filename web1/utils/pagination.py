...
def get_pager_info(Model, filter_query, url, id, per_items=12):
    ...
    if url == '/api/file_upload/':
        if belong_project != 'All':
            obj = obj.filter(project__project_name__contains=belong_project)

        elif belong_module != '请选择':
            obj = obj.filter(module__module_name__contains=belong_module)
        ...
        if filter_query.get('author'):
            obj = obj.filter(report_name__contains=filter_query.get('report_name'), author__contains=filter_query.get('author'))
        else:
        ...
        if belong_project != 'All':
            obj = obj.filter(belong_project__project_name__contains=belong_project)
        elif ca_user and ca_user != 'All':
            obj = obj.filter(user_name=ca_user)
