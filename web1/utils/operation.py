...
def add_test_reports(runner, author="autoRun", report_name=None):
    """
    定时任务或者异步执行报告信息落地
...
def del_file_data(id):
    ...
    try:
        ...
        objfile = os.path.join(os.getcwd(), file_path)
...
def add_ca_data(**kwargs):
    try:
        ca_loginUrl = CaCFG.objects.get().ca_loginurl
        ca_url = CaCFG.objects.get().ca_url
        ...
        ca, expire_time = get_ca(ca_loginUrl, ca_url, user_name, password, client_id, redirect_url)
        ca_info = {}
        ca_info["user_name"] = user_name
        ca_info["password"] = password
        ca_info['belong_project'] = ProjectInfo.objects.get(project_name=project_name)
        ca_info["author"] = kwargs.pop('author')
        ca_info["value"] = ca
        ca_info["client_id"] = client_id
        ca_info["redirect_url"] = redirect_url
        ca_info["expire_time"] = datetime.datetime.now() + datetime.timedelta(milliseconds=float(expire_time))
        ca_info["status"] = True if ca else False
...
def update_ca_url_data(ca_url, ca_loginurl):
    try:
        CaCFG.objects.filter(id=1).update(ca_url=ca_url, ca_loginurl=ca_loginurl)
    except:
...
def get_ca(ca_loginurl, ca_url, username, password, client_id, redirect_url):
    try:
        ...
        resp = requests.post(ca_loginurl, data=data, allow_redirects=False)
        print(resp.content)
        ...
        resp = requests.get(ca_url, params, headers=headers)
        print(resp.url)
        pa = re.compile(r"#id_token=(.*)&access_token.*expires_in=(.*)$")
